        ...
        for row in csv_reader:
            if site_type == "tos":
                ...
                if url:
                    ...
                    urls.add(url)
    if not urls:
        logger.error(f"No 'URL' column found in the CSV file: {csv_file_path}")
        raise ValueError(f"No 'URL' column found in the CSV file: {csv_file_path}")
    return list(urls)


def normalize_url(url):
    """Normalizes URL by ensuring it has 'http://' and ends with '/robots.txt'."""
    ...
    if current_chunk:
        if len(output_files) == 0:
            with output_json_path.open("w", encoding="utf-8") as file:
                json.dump(
                    current_chunk, file, ensure_ascii=False, indent=4, default=str
                )
            ...
            with chunk_file_path.open("w", encoding="utf-8") as chunk_file:
                json.dump(
                    current_chunk, chunk_file, ensure_ascii=False, indent=4, default=str
                )
            output_files.append(chunk_file_path)
...
def extract_and_format_text(file_path: Path) -> str:
    ...
    """
    try:
        # Read in binary mode first to detect encoding
        with file_path.open("rb") as file:
            raw_data = file.read()
            # Use more reliable encoding detection with confidence threshold
        ...
        # Read with detected encoding but handle decoding errors gracefully
        with file_path.open("r", encoding=detected_encoding, errors="replace") as file:
            soup = BeautifulSoup(file, "html.parser", from_encoding=detected_encoding)
...
def create_biannual_json(json_directory: str, output_file: str) -> None:
    """
